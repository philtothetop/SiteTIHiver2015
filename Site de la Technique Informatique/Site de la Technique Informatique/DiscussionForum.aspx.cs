    ...
    public partial class DiscussionForum : ErrorHandling
    {
        ...
        protected void lviewDiscussion_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            using (LeModelTIContainer lecontexte = new LeModelTIContainer())
            {
                ...
                {
                    imgPhotoProfil.ImageUrl = isLocal() ? "~/Upload/Photos/Profils/" + membre.pathPhotoProfil : "~/../Upload/Photos/Profils/" + membre.pathPhotoProfil; //"~/Photos/Profils/" + membre.pathPhotoProfil;
                }
                else
                {
                    imgPhotoProfil.ImageUrl = isLocal() ? "~/Upload/Photos/Profils/photobase.bmp" : "~/../Upload/Photos/Profils/photobase.bmp"; //"~/Photos/Profils/photobase.bmp";
                }
            }
        }
